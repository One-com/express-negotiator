...
function getVariantInfoFromExtensionString(extensionString, supportUserAgent) {
    var variantInfo = {
        ...
        unmatchedExtensionsWithoutLeadingDot: []
    };
    if (supportUserAgent) {
...
module.exports = function (options) {
    ...
    var roots = Array.isArray(options.root) ? options.root : [options.root],
        ...
        getVariantInfosByBaseName = memoizeAsync(function (rootRelativeDirName, cb) {
            ...
            async.eachLimit(roots.map(function (root) {
                ...
                fs.readdir(dirName, function (err, fileNames) {
                    ...
                    if (options.watch && !(dirName in fsWatcherByDirName)) {
                        try {
                            ...
                            // Might fail with a ENOSPC error on certain systems. Tell the caller so the memoizer can be purged immediately
                            stale = true;
                        }
                    }
                    var statByFileName = {};
                ...
                cb(null, variantInfosByBaseName, stale);
            }));
        });
    ...
    return function expressNegotiator(req, res, next) {
        ...
        if (matchDirAndFileName) {
            var baseName,
                extensionString;
            if (matchDirAndFileName[2]) {
                try {
                    baseName = decodeURIComponent(matchDirAndFileName[2]);
                } catch (e) {
                    return next();
                }
            } else {
            ...
            if (matchDirAndFileName[3]) {
                try {
                    extensionString = decodeURIComponent(matchDirAndFileName[3]);
                } catch (e2) {
                    return next();
                }
            } else {
            ...
            return getVariantInfosByBaseName(dirName, passError(next, function (variantInfosByBaseName, stale) {
                if (stale) {
                    getVariantInfosByBaseName.purge(dirName);
                }
                            ...
                            if (variantInfo.fileName === fileName) {
                                ...
                                if (!variantInfo.localeId || localeIdsEqual(variantInfo.localeId, localeId)) {
                                    ...
                                    if (contentTypeQuality > 0) {
                                        // Find if new match is better than previous
                                        var newIsBetter = false;
                                        if (bestVariantInfo) {
                                            if ((contentTypeQuality > bestVariantQuality) ||
