...
module.exports = function (options) {
    ...
    var fsWatcherByDirName = {};

    var getVariantInfosByBaseName = memoizeAsync(function (dirName, cb) {
        fs.readdir(dirName, function (err, fileNames) {
            ...
            async.eachLimit(fileNames, 20, function (fileName, cb) {
                fs.lstat(Path.resolve(dirName, fileName), function (err, stats) {
                    if (err) {
                        if (err.code === 'ENOENT') {
    ...
    return function expressNegotiator(req, res, next) {
        ...
        if (matchDirAndFileName) {
            ...
            return getVariantInfosByBaseName(Path.resolve(options.root, dirName.substr(1)), passError(next, function (variantInfosByBaseName) {
                ...
                if (variantInfos) {
                    ...
                    if (bestVariantInfo) {
                        ...
                        bestVariantInfo.absolutePath = Path.resolve(options.root, targetUrl.substr(1).replace(/\?.*$/, ''));
