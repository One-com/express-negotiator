                        ...
                        for (var j = 0 ; j < variantInfos.length ; j += 1) {
                            ...
                            if (variantInfo.fileName === fileName) {
                                ...
                                if (!variantInfo.localeId || variantInfo.localeId === localeId) {
                                    ...
                                    if (contentTypeQuality > 0 && (!bestVariantInfo || contentTypeQuality > bestVariantQuality) || (contentTypeQuality === bestVariantQuality && variantInfo.contentType === 'text/html')) {
                    ...
                    if (bestVariantInfo) {
                        ...
                        if (bestVariantInfo.localeId) {
                            return fs.stat(Path.resolve(options.root, targetUrl.substr(1).replace(/\?.*$/, '')), passError(next, function (stat) {
                                res.setHeader('ETag', '"' + bestVariantInfo.localeId + '-' + stat.size + '-' + Number(stat.mtime) + '"');
