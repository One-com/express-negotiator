...
module.exports = function (options) {
    ...
    var getPrioritizedLocaleIds = memoizer(function (localeParameterValue, localeCookieValue, acceptLanguageHeaderValue) {
        var prioritizedLocaleIds = [];
        if (localeParameterValue) {
            prioritizedLocaleIds.push(localeParameterValue);
        }
        ...
        if (req.accepts('html') || /(?:^|,| )(?:text|\*)\/(?:\*|html)(?:,| |$)/.test(req.headers.accept)) {
            ...
            if (matchDirAndFileName) {
                var dir = matchDirAndFileName[1],
                    ...
                    prioritizedLocaleIds = getPrioritizedLocaleIds(matchLocaleParameterValue && matchLocaleParameterValue[1],
                                                                   localeCookieValue,
